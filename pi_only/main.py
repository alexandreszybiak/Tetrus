...
def main():
    ...
    if not PI:
        ...
        device.contrast(20) # Manage the brightness of the max7219 8x8 matrix (0 - 255)
        pygame.init()
        drawImage('/home/pi/pi.bmp')
        pygame.joystick.init()
        while joystick_detected==False:
            show_message(device,"Waiting for controller...",fill="white", font=proportional(LCD_FONT), scroll_delay=0.01)
    ...
    drawClock(1)
    if PI:
        show_message(device,"Let's play",fill="white", font=proportional(LCD_FONT))
...
def runSnakeGame():
    ...
    if os.path.isfile('/home/pi/hs_snake.p')==True:
        ...
        highscore=0
    if PI:
        show_message(device,"Snake Highscore: " + str(highscore),fill="white", font=proportional(LCD_FONT), scroll_delay=0.01)
        ...
        if wormCoords[HEAD]['x'] == -1 or wormCoords[HEAD]['x'] == BOARDWIDTH or wormCoords[HEAD]['y'] == -1 or wormCoords[HEAD]['y'] == BOARDHEIGHT:
            ...
            if score > highscore:
                ...
                if PI:
                    ...
                    show_message(device,"New Highscore !!!",fill="white", font=proportional(LCD_FONT), scroll_delay=0.01)
            return # game over
...
def runTetrisGame():
    ...
    if os.path.isfile('/home/pi/hs_tetris.p')==True:
        ...
        highscore=0
    if PI:
        show_message(device,"Tetris Highscore: " + str(highscore),fill="white", font=proportional(LCD_FONT), scroll_delay=0.01)
...
def shutdownScreen():
    ...
    if PI:
        ...
        show_message(device,"Press Select to shutdown!",fill="white", font=proportional(LCD_FONT), scroll_delay=0.01)
    else:
            ...
            if event.type == pygame.JOYBUTTONDOWN or event.type == KEYDOWN:
                ...
                if (myevent!=JKEY_SEL):
                    ...
                    if not PI:
                        ...
                        clearScreen()
                        updateScreen()
                        show_message(device,"Shutdown...",fill="white", font=proportional(LCD_FONT), scroll_delay=0.01)
